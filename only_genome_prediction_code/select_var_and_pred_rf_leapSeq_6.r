...
for (varNum in seq(2, 100, 2))
{
  ...
  Fit <- train(Yield ~ .,
    ...
    tuneGrid = expand.grid(nvmax=TUNE
    ),
    metric = "R2",
  )

  R_Fit <- sapply(1:(Repeats * Numbers), FUN = function(x) {
    cor(predict(Fit, newdata = impute_Bin_matrix[Fit$control$index[[x]], ]), Y[Fit$control$index[[x]]]) # nolint
  })
...
write.table(Result_All, paste0("~/machine_learning_project/bin_inf_predict_yield/FS_result/", MM, "_nvmax", TUNE, "_1_29_results_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
)

write.table(Resample_All, paste0("~/machine_learning_project/bin_inf_predict_yield/FS_result/", MM,"_nvmax", TUNE, "_1_29_resample_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(as.data.frame(do.call(cbind, Fit$control$index)),
  paste0("~/machine_learning_project/bin_inf_predict_yield/FS_result/", MM, "_nvmax", TUNE,"_1_29_train_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(IMP_Overall, paste0("~/machine_learning_project/bin_inf_predict_yield/FS_result/", MM, "_nvmax", TUNE, "_1_29_IMP_Overall_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
)
