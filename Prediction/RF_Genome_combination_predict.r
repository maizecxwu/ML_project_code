...
print('omics_type 2: Phenome, metabolome, Genome, Early_metabolome')
print('omics_type 3: Phenome, metabolome, Genome, Early_metabolome')
stop('Try again!')
}
...
assign(
str_c("impuated_Phenome"), read_X_data("Phenome")
)
assign(
str_c("impuated_metabolome"), read_X_data("metabolome")
)
assign(
str_c("impuated_Genome"), read_X_data("Genome")
)

read_Feature_data <- function(Select_M, OMICSS) {
read.table(
str_c("~/machine_learning_project/train_again/",
Select_M, "_", OMICSS, "_mean_IMP_result.txt"),
header = T, sep = "\t", check.names = F)
}

assign(
str_c(args [1], "_Var_feature_Genome"), read_Feature_data(args [1], "Genome")
)
assign(
str_c(args [1], "_Var_feature_metabolome"), read_Feature_data(args [1], "metabolome")
)
assign(
str_c(args [1], "_Var_feature_Phenome"), read_Feature_data(args [1], "Phenome")
)
...
ALL_params <- get_model_params_combn(args[3])
...
#  tryCatch({
  ...
  get_any_omics_top20_var_dat <- function(OMICSS) {
      get(str_c('impuated_', OMICSS)) %>% select(
      get(str_c(args [1], '_Var_feature_', OMICSS)) %>%
      head(varNum) %$% var
    )
  }

  ALL_Omics_VAR_df <-
  map_dfc(args [c(2, 5, 6)] %>% na.omit(), get_any_omics_top20_var_dat)

  df <- cbind(ALL_Omics_VAR_df, Y)
...
clusterExport(cl = cl, varlist =
  ...
  str_c(args [1], "_Var_feature_", c(All_omics)) %>% as.character(),
  str_c('impuated_', All_omics)) %>% as.list()
)
...
str_c(args [c(1, 2, 5, 6, 4)] %>% na.omit, collapse = '_'), "_predict_result")
...
# write.table(
# data.frame(
#     FS_method = rep(c("RF"), 9),
#     Omics_type = rep(c('Phenome', 'metabolome', 'Genome'), 3),
#     Model =
#       rep(c('rf', "gaussprRadial", "pls"), each = 3),
#     # Trait = rep(c('PH', 'EH', 'DTT', 'DTA', 'DTS'), each = 9),
#     Omics_type2 = rep(c('Genome', 'Phenome', 'metabolome'), 3)
# ) %>% . [rep(1:9, 6), ] %>%
# mutate(Trait = rep(c('Yield', 'PH', 'EH', 'DTT', 'DTA', 'DTS'), each = 9)) %>%
# select(FS_method, Omics_type, Model, Trait, Omics_type2)
...
# data.frame(
#     FS_method = rep(c("RF"), 9),
#     Omics_type = rep(c('Phenome'), 3),
#     Model =
#       rep(c('rf', "gaussprRadial", "pls"), each = 3),
#     # Trait = rep(c('PH', 'EH', 'DTT', 'DTA', 'DTS'), each = 9),
#     Omics_type2 = rep(c('Genome'), 3)
# ) %>% . [rep(1:9, 6), ] %>%
# mutate(Trait = rep(c('Yield', 'PH', 'EH', 'DTT', 'DTA', 'DTS'), each = 9)) %>%
# select(FS_method, Omics_type, Model, Trait, Omics_type2) %>%
# mutate(Omics_type3 = rep(c('metabolome'), 54)) %>% distinct(),
