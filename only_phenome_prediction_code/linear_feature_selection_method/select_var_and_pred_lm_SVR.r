...
for (varNum in 2:120)
{
  ...
  if (varNum == 1) {
    df <- cbind(Y, impuated_X[, na.omit(NameofVar)])
    colnames(df) <- c("Yield", na.omit(NameofVar))
  } else {
    df <- cbind(Y, impuated_X[, na.omit(NameofVar)]) %>%
...
write.table(Result, paste0("~/machine_learning_project/feature_selection_recursive/different_numsofvars_pred/lm_selection_methods_Result/", MM, "_results_lm.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(Resample, paste0("~/machine_learning_project/feature_selection_recursive/different_numsofvars_pred/lm_selection_methods_Result/", MM, "_resample_lm.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(as.data.frame(do.call(cbind, Fit$control$index)),
  paste0("~/machine_learning_project/feature_selection_recursive/different_numsofvars_pred/lm_selection_methods_Result/", MM, "_train_lm.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(IMP_Overall, paste0("~/machine_learning_project/feature_selection_recursive/different_numsofvars_pred/lm_selection_methods_Result/", MM, "_IMP_Overall_lm.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 
