...
# enet :
TUNE_Overal <-
  cbind(rep(c(0,1e-06,1e-05,1e-04,1e-03,1e-02,1e-01,1),each=10),
        rep(seq(0.1,1,0.1),8)) %>% as.data.frame %>%
  rename(lambda=V1,fraction=V2)
...
write.table(Result_All, paste0("~/machine_learning_project/feature_tune_parallel/lm_model_results/", MM, "_results_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(Resample_All, paste0("~/machine_learning_project/feature_tune_parallel/lm_model_results/", MM, "_resample_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(as.data.frame(do.call(cbind, Fit$control$index)),
  paste0("~/machine_learning_project/feature_tune_parallel/lm_model_results/", MM, "_train_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
)

write.table(IMP_Overall, paste0("~/machine_learning_project/feature_tune_parallel/lm_model_results/", MM, "_IMP_Overall_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
)
