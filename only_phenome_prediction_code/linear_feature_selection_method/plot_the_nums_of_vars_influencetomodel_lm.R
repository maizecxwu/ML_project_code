...
for (nvmax in seq(2, 110, 2))
{
    Result <- data.frame()
    Resample <- data.frame()
    IMP_Overall <- data.frame()

    for (varNum in 2:110) # if is the lmFuncs, the vars must be lower than 110
    {
        NameofVar <-
            lm_var %>%
        ...
        Fit <- train(Yield ~ .,
            ...
            tuneGrid = expand.grid(nvmax = nvmax),
            metric = "R2",
        )
    ...
    write.table(Result, paste0("~/machine_learning_project/use_stepwisefromZhang_to_validation/pdf/diff_NumsofVars_influence/lm_result/", MM, nvmax, "_2_110_results_lm.txt"),
        row.names = F,
        col.names = T, sep = "\t", quote = F
    ) 

    write.table(Resample, paste0("~/machine_learning_project/use_stepwisefromZhang_to_validation/pdf/diff_NumsofVars_influence/lm_result/", MM, nvmax, "_2_110_resample_lm.txt"),
        row.names = F,
        col.names = T, sep = "\t", quote = F
    ) 

    write.table(as.data.frame(do.call(cbind, Fit$control$index)),
        paste0("~/machine_learning_project/use_stepwisefromZhang_to_validation/pdf/diff_NumsofVars_influence/lm_result/", MM, nvmax, "_2_110_train_lm.txt"),
        row.names = F,
        col.names = T, sep = "\t", quote = F
    ) 

    write.table(IMP_Overall, paste0("~/machine_learning_project/use_stepwisefromZhang_to_validation/pdf/diff_NumsofVars_influence/lm_result/", MM, nvmax, "_2_110_IMP_Overall_lm.txt"),
        row.names = F,
        col.names = T, sep = "\t", quote = F
    )
}
