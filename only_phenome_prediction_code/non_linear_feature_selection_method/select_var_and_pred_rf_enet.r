...
write.table(Result_All, paste0("~/machine_learning_project/feature_tune_parallel/5_model_results/", MM, "_results_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(Resample_All, paste0("~/machine_learning_project/feature_tune_parallel/5_model_results/", MM, "_resample_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
)

write.table(as.data.frame(do.call(cbind, Fit$control$index)),
  paste0("~/machine_learning_project/feature_tune_parallel/5_model_results/", MM, "_train_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
)

write.table(IMP_Overall, paste0("~/machine_learning_project/feature_tune_parallel/5_model_results/", MM, "_IMP_Overall_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
)
