...
for (nvmax in seq(2, 120, 2))
{
    Result <- data.frame()
    Resample <- data.frame()
    IMP_Overall <- data.frame()

    for (varNum in 2:120) # if is the lmFuncs, the vars must be lower than 110
    {
        NameofVar <-
            lm_var %>%
    ...
    write.table(Result, paste0("~/machine_learning_project/use_stepwisefromZhang_to_validation/pdf/diff_NumsofVars_influence/result/", MM, nvmax, "_2_120_results_lm.txt"),
        row.names = F,
        col.names = T, sep = "\t", quote = F
    )

    write.table(Resample, paste0("~/machine_learning_project/use_stepwisefromZhang_to_validation/pdf/diff_NumsofVars_influence/result/", MM, nvmax, "_2_120_resample_lm.txt"),
        row.names = F,
        col.names = T, sep = "\t", quote = F
    ) 

    write.table(as.data.frame(do.call(cbind, Fit$control$index)),
        paste0("~/machine_learning_project/use_stepwisefromZhang_to_validation/pdf/diff_NumsofVars_influence/result/", MM, nvmax, "_2_120_train_lm.txt"),
        row.names = F,
        col.names = T, sep = "\t", quote = F
    ) 

    write.table(IMP_Overall, paste0("~/machine_learning_project/use_stepwisefromZhang_to_validation/pdf/diff_NumsofVars_influence/result/", MM, nvmax, "_2_120_IMP_Overall_lm.txt"),
        row.names = F,
        col.names = T, sep = "\t", quote = F
    ) 
}
