...
write.table(Result_All, paste0("~/machine_learning_project/feature_tune_parallel/5_model_results/", MM, "_62_120_results_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(Resample_All, paste0("~/machine_learning_project/feature_tune_parallel/5_model_results/", MM, "_62_120_resample_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
)

write.table(as.data.frame(do.call(cbind, Fit$control$index)),
  paste0("~/machine_learning_project/feature_tune_parallel/5_model_results/", MM, "_62_120_train_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(IMP_Overall, paste0("~/machine_learning_project/feature_tune_parallel/5_model_results/", MM, "_62_120_IMP_Overall_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 
