...
impute_Bin_matrix <- A.mat(Bin_matrix,
  max.missing = 0.5,
...
lm_var_2 <- read.table("~/machine_learning_project/Meta_analysis/Early_Meta_rfFuncs_rm_0_variables.txt",
  header = T, check.names = F, sep = "\t"
)   %>%
...
for(TUNE in c(seq(0.002,0.01,0.001), seq(0.02,0.1,0.01)))
{
...
if (varNum == 1) {
  ...
  df <- cbind(
    ...
    impute_Bin_matrix[, NameofVar], 
    impuated_Meta[, lm_var_2]
  ...
  Fit <- train(Yield ~ .,
    ...
    tuneGrid = expand.grid(sigma=TUNE
    ),
    metric = "R2",
  )
...
write.table(Result_All, paste0("~/machine_learning_project/Early_Phenomics_Bin_Meta/result/", MM, "_", VARNUM, "_Meta_1_10_results_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(Resample_All, paste0("~/machine_learning_project/Early_Phenomics_Bin_Meta/result/", MM, "_", VARNUM, "_Meta_1_10_resample_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table(as.data.frame(do.call(cbind, Fit$control$index)),
  paste0("~/machine_learning_project/Early_Phenomics_Bin_Meta/result/", MM, "_", VARNUM, "_Meta_1_10_train_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
) 

write.table( IMP_Overall_All , paste0("~/machine_learning_project/Early_Phenomics_Bin_Meta/result/", MM, "_", VARNUM, "_Meta_1_10_IMP_Overall_rf.txt"),
  row.names = F,
  col.names = T, sep = "\t", quote = F
)
